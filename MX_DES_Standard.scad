...
/*
mirror([0,0,0])keycap(
  keyID  = keyID1, //change profile refer to KeyParameters Struct
...
//FFwd1 FFwd2 FPit1 FPit2  DshDep DshHDif FArcIn FArcFn FArcEx     BFwd1 BFwd2 BPit1 BPit2  BArcIn BArcFn BArcEx
  ...
  // modified low pro 3 row system
  [   6,    3,   18,  -50,      5,    1.8,   9.8,    15,     2,        5,  4.4,    5,  -55,    9.8,    15,     2], //R4
  [   5,  3.5,   10,  -55,      5,    1.8,   9.5,    15,     2,        6,  3,     10,  -55,    9.5,    15,     2], //R3
  [   6,    3,   10,  -50,      5,    1.8,   9.8,    15,     2,        6,    4,   13,   30,    9.8,    16,     2], //R2
  [   6,    3,   -5,  -50,      5,    1.8,   9.8,    15,     2,        6,  3.5,   13,  -50,    9.8,    15,     2], //R5 mod
...
function BottomWidth(keyID)  = $bottom_key_width * keyParameters[keyID][1];  //
function BottomLength(keyID) = $bottom_key_height * keyParameters[keyID][2];  //
function TopWidthDiff(keyID) = keyParameters[keyID][3];  //
function TopLenDiff(keyID)   = keyParameters[keyID][4];  //
function KeyHeight(keyID)    = keyParameters[keyID][5];  //
function TopWidShift(keyID)  = keyParameters[keyID][6];
function TopLenShift(keyID)  = keyParameters[keyID][7];
function XAngleSkew(keyID)   = keyParameters[keyID][8];
function YAngleSkew(keyID)   = keyParameters[keyID][9];
function ZAngleSkew(keyID)   = keyParameters[keyID][10];
function WidExponent(keyID)  = keyParameters[keyID][11];
function LenExponent(keyID)  = keyParameters[keyID][12];
function CapRound0i(keyID)   = keyParameters[keyID][13];
function CapRound0f(keyID)   = keyParameters[keyID][14];
function CapRound1i(keyID)   = keyParameters[keyID][15];
...
///----- KEY Builder Module
module keycap(keyID = 0, cutLen = 0, visualizeDish = false, Dish = true, Stem = true, crossSection = false, Legends = false, homeDot = false, Stab = 0) {
...
function keyIDbyName(name) = search([name], keyParameters)[0];
